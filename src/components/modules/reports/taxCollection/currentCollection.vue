      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-info">
            ...
            <h4 class="card-title"> {{credentials['union']}} ইউনিয়ন পরিষদ হাল কর আদায়ের রেজিষ্টার প্রতিবেদন </h4>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col-md-2">
                  ...
                  <tr>
                    <td>
                      <div class="col-md-12"  style="text-align:center">
                         <br>
                        <h5>{{credentials['union']}} ইউনিয়ন পরিষদ হাল কর আদায়ের রেজিষ্টার প্রতিবেদন, <span v-if="credentials['village']"> {{credentials['village']}}, </span> <span v-if="(credentials['word'])"> {{credentials['word']}} নং ওয়ার্ড, </span> <span v-if="(credentials['year'])"> {{credentials['year']}}</span>({{credentials['type']}})</h5>
                ...
                <table class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <tr v-for="data in allHoldings.data" :key="data.id">
                      ...
                      <td> {{data.tax}} </td>
                      <td> {{data.total_paid-data.prev_paid}} </td>
                      <td> {{data.last_payment_date}} </td>
                    </tr>
                    <tr style="background:#999">
                      <th colspan="7" style="text-align:right">মোট:</th>
                      <th>{{this.footer.total_tax}}</th>
                      <th>{{this.footer.total_tax_paid}}</th>
                      <th></th>
                    </tr>
                  </tbody>
...
export default {
  ...
  methods: {
    async getData () {
       const yearsData = await ReportsService.years()
    ...
    async holdings(page = 1){
      ...
      ReportsService.currentTaxCollection(this.form).then(response=>{
    ...
    dataDistribute(){
      this.footer = {
        ...
        for(let i = 0;i<this.allData.length;i++){
          this.footer.total_tax+=parseFloat(this.allData[i].tax)
          this.footer.total_tax_paid += parseFloat(this.allData[i].total_paid-this.allData[i].prev_paid)
